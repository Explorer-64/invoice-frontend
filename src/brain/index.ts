...
const constructBaseUrl = (): string => {
  // If API_URL is set and API_HOST is empty, use API_URL directly (local development)
  if (API_URL && !API_HOST) {
    return API_URL;
  }
  ...
  return new Brain({
    ...
    customFetch: (url, options) => {
      if (isDeployedToCustomApiPath) {
        ...
        // another domain with custom API path
    ...
    securityWorker: async () => {
      ...
      console.log('[Brain] Security worker called, auth header:', authHeader ? authHeader.substring(0, 50) + '...' : 'MISSING');
      return {
        headers: {
